...
## Build Instructions
This repository is configured for use with Visual Studio using msbuild/solutions and is currently ONLY compatible with **Visual Studio 2013**. To setup your environment, acquire any edition of Visual Studio 2013 and clone this repository to your machine. Open Visual Studio 2013 and use `File > Open > Project/Solution` and navigate to the path where you cloned the repository then select the solution file located at `src/build/win32/swg.sln`. Current modernization targets standardize external/runtime packaging on 64-bit outputs.
For a focused 64-bit renderer build checklist (including Direct3D 9/9Ex outputs), see `docs/win32-client-build.md`.

...
void MapView::drawRectangle_w (CDC* pDC, const Vector2d& start, const Vector2d& end, const VectorArgb& color) const
{
	...
	int left   = std::min(startScreen.x, endScreen.x);
	int right  = std::max(startScreen.x, endScreen.x);
	int top    = std::min(startScreen.y, endScreen.y);
	int bottom = std::max(startScreen.y, endScreen.y);
...
void MapView::OnDraw(CDC* pDC)
{
	...
	Graphics::setViewport (0, 0, std::min(Graphics::getCurrentRenderTargetWidth(), rect.Width ()), std::min(Graphics::getCurrentRenderTargetHeight(), rect.Height ()));

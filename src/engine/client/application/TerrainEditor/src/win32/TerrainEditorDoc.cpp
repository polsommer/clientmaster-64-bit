...
BEGIN_MESSAGE_MAP(TerrainEditorDoc, CDocument)
	...
	ON_UPDATE_COMMAND_UI(ID_NEW_FILTER_SHADER, OnUpdateNewFilter)
        ...
        ON_COMMAND(ID_AI_AUTOGENERATE, OnAiAutogenerate)
        ON_UPDATE_COMMAND_UI(ID_AI_AUTOGENERATE, OnUpdateAiAutogenerate)
        ON_COMMAND(ID_AI_RUNAUDIT, OnAiRunAudit)
        ON_UPDATE_COMMAND_UI(ID_AI_RUNAUDIT, OnUpdateAiRunAudit)
        //}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
TerrainEditorDoc::TerrainEditorDoc() :
	...
	profileFrame (0),
        ...
        m_guidanceOverlayEnabled(false),
        m_heatmapPreviewEnabled(false),
        m_guidelineLayerEnabled(false),
        m_autoShaderAssignmentEnabled(true),
        m_autoGenerationResult()
{
...
TerrainEditorDoc::AutoGenerationResult::AutoGenerationResult()
:       valid(false)
{
}
...
BOOL TerrainEditorDoc::OnNewDocument()
{
	...
	defaultShaderSize          = CONST_REAL (2);
        ...
        TerrainAutoPainter::Config autoConfig;
        autoConfig.seed = static_cast<int>(Os::getRealSystemTime() & 0x7fffffff);
        autoConfig.gridSize = 257;
        autoConfig.enableShaderAssignment = m_autoShaderAssignmentEnabled;

        AutoGenerationResult autoResult;
        autoResult.report = TerrainAutoPainter::generateAndApply(*this, autoConfig);
        autoResult.valid = true;
        setAutoGenerationResult(autoResult);
...
bool TerrainEditorDoc::isGuidanceOverlayEnabled() const
{
...
bool TerrainEditorDoc::isHeatmapPreviewEnabled() const
{
...
bool TerrainEditorDoc::isGuidelineLayerEnabled() const
{
...
bool TerrainEditorDoc::isAutoShaderAssignmentEnabled() const
{
        return m_autoShaderAssignmentEnabled;
}
...
void TerrainEditorDoc::setAutoShaderAssignmentEnabled(bool enabled)
{
        if (m_autoShaderAssignmentEnabled != enabled)
        {
                m_autoShaderAssignmentEnabled = enabled;
                ...
                if (!enabled)
                {
                        ...
                        if (generator)
                        {
                                for (int i = generator->getNumberOfLayers() - 1; i >= 0; --i)
                                {
                                        TerrainGenerator::Layer *const layer = generator->getLayer(i);
                                        if (layer && layer->getName() && _stricmp(layer->getName(), "AI Shader Assignment") == 0)
                                                generator->removeLayer(layer, true);
                                }
                        }
                }
...
void TerrainEditorDoc::setAutoGenerationResult(const AutoGenerationResult &result)
{
        m_autoGenerationResult = result;
        UpdateAllViews(0);
}

//-------------------------------------------------------------------

const TerrainEditorDoc::AutoGenerationResult &TerrainEditorDoc::getAutoGenerationResult() const
{
        return m_autoGenerationResult;
}

//-------------------------------------------------------------------

bool TerrainEditorDoc::hasAutoGenerationResult() const
{
        return m_autoGenerationResult.valid;
}
...
void TerrainEditorDoc::OnAiAutogenerate()
{
        if (!getTerrainGenerator())
                return;
        ...
        autoConfig.seed = static_cast<int>(Os::getRealSystemTime() & 0x7fffffff);
        autoConfig.gridSize = 257;
        autoConfig.desiredSettlementCount = std::max(8, autoConfig.desiredSettlementCount);
        autoConfig.riverCount = 5;
        autoConfig.logisticsHubCount = std::max(4, autoConfig.logisticsHubCount);
        autoConfig.enableBiomeRebalancing = true;
        autoConfig.enableSettlementZoning = true;
        autoConfig.enableTravelCorridorPlanning = true;
        autoConfig.enableLightingDirector = true;
        autoConfig.enableWeatherSynthesis = true;
        autoConfig.enableEncounterScripting = true;
        autoConfig.enableCinematicMoments = true;
        autoConfig.enableShaderAssignment = m_autoShaderAssignmentEnabled;
        ...
        TerrainEditorApp *const app = GetApp();
        if (app)
                app->showConsole();

        CString consoleLine;
        consoleLine.Format(_T("\r\n[Auto terrain] Seed %d complete – %u toolkit hooks, %u corridors, %u encounters.\r\n"),
                autoConfig.seed,
                static_cast<unsigned>(autoResult.report.automationToolkit.size()),
                static_cast<unsigned>(autoResult.report.travelCorridors.size()),
                static_cast<unsigned>(autoResult.report.encounterScripts.size()));
        addConsoleMessage(consoleLine);

        if (!autoResult.report.aiStatusHeadline.IsEmpty())
                addConsoleMessage(autoResult.report.aiStatusHeadline + _T("\r\n"));

        if (!autoResult.report.operationsChecklist.IsEmpty())
                addConsoleMessage(autoResult.report.operationsChecklist + _T("\r\n"));
}

//-------------------------------------------------------------------

void TerrainEditorDoc::OnUpdateAiAutogenerate(CCmdUI* pCmdUI)
{
        pCmdUI->Enable(getTerrainGenerator() != 0);
}

//-------------------------------------------------------------------

void TerrainEditorDoc::OnAiRunAudit()
{
        if (!getTerrainGenerator())
        {
                AfxMessageBox(_T("Load a terrain document before running the smart audit."));
                return;
        }
        ...
        addConsoleMessage(_T("\r\n[Smart Terrain Audit]\r\n"));
        addConsoleMessage(audit);
}

//-------------------------------------------------------------------

void TerrainEditorDoc::OnUpdateAiRunAudit(CCmdUI* pCmdUI)
{
        pCmdUI->Enable(getTerrainGenerator() != 0);
}

...
#ifndef min
#define TERRAIN_EDITOR_DEFINED_MIN 1
#define min(a, b) ((a) < (b) ? (a) : (b))
#endif

#ifndef max
#define TERRAIN_EDITOR_DEFINED_MAX 1
#define max(a, b) ((a) > (b) ? (a) : (b))
#endif

#include <atlimage.h>

#ifdef TERRAIN_EDITOR_DEFINED_MIN
#undef min
#undef TERRAIN_EDITOR_DEFINED_MIN
#endif

#ifdef TERRAIN_EDITOR_DEFINED_MAX
#undef max
#undef TERRAIN_EDITOR_DEFINED_MAX
#endif
...
namespace
{
	bool loadPngImageList(CImageList& imageList, UINT resourceId, int imageSize)
	{
		HINSTANCE instanceHandle = AfxGetResourceHandle();
		HRSRC resourceHandle = ::FindResource(instanceHandle, MAKEINTRESOURCE(resourceId), _T("PNG"));
		if (!resourceHandle)
			return false;

		HGLOBAL resourceDataHandle = ::LoadResource(instanceHandle, resourceHandle);
		if (!resourceDataHandle)
			return false;

		DWORD resourceSize = ::SizeofResource(instanceHandle, resourceHandle);
		const void* resourceData = ::LockResource(resourceDataHandle);
		if (!resourceData || resourceSize == 0)
			return false;

		HGLOBAL streamHandle = ::GlobalAlloc(GMEM_MOVEABLE, resourceSize);
		if (!streamHandle)
			return false;

		void* streamData = ::GlobalLock(streamHandle);
		if (!streamData)
		{
		...
		memcpy(streamData, resourceData, resourceSize);
		::GlobalUnlock(streamHandle);

		IStream* stream = nullptr;
		if (FAILED(::CreateStreamOnHGlobal(streamHandle, TRUE, &stream)))
		{
			::GlobalFree(streamHandle);
			return false;
		}
		...
		CImage image;
		HRESULT loadResult = image.Load(stream);
		stream->Release();
		if (FAILED(loadResult))
			return false;

		imageList.DeleteImageList();
		if (!imageList.Create(imageSize, imageSize, ILC_COLOR32, 0, 1))
			return false;
		...
		if (imageList.Add(&bitmap, static_cast<CBitmap*>(nullptr)) == -1)
			return false;
...
int MainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	int imageSize = 32;
	UINT imageResource = IDR_MAINFRAME;
	CImageList* imageList = &m_toolBarImages32;

	if (dpi >= 192)
	{
		imageSize = 96;
		imageResource = IDB_MAINFRAME_PNG_96;
		imageList = &m_toolBarImages96;
	}
	else if (dpi >= 144)
	{
		imageSize = 64;
		imageResource = IDB_MAINFRAME_PNG_64;
		imageList = &m_toolBarImages64;
	}

	if (loadPngImageList(*imageList, imageResource, imageSize))
	{
		m_wndToolBar.GetToolBarCtrl().SetImageList(imageList);
		m_wndToolBar.SetSizes(CSize(imageSize + 7, imageSize + 7), CSize(imageSize, imageSize));
	}

...
void CNewTreeListCtrl::RecalcHeaderPosition()
{
	if(m_RTL)
	{
		...
		if(GetColumnsWidth() > m_clientRect.Width())
		{
			...
			m_wndHeader.SetWindowPos(&wndTop, m_nOffset, 0, std::max(pFrame->StretchWidth(GetColumnsWidth(),m_clientRect.Width()),m_wndRect.Width()), m_headerRect.Height(), SWP_SHOWWINDOW);
		}
	}
}
...
void CNewTreeListCtrl::OnPaint() 
{
	...
	while(hItem!=NULL && n>=0)
	{
		...
		if ( !(GetItemState( hItem, selflag ) & selflag ))
		{
			...
			if(GetColumnsNum()>1)
				rect.left = std::min(static_cast<int> (m_labelRect.left), static_cast<int> (GetColumnWidth(0)));
			else
				...
				// SetWindowPos
			{
					...
					// hide the horz scroll bar and update the tree
				{
					...
					// set scroll offset to zero
					{
						...
						m_wndHeader.SetWindowPos(&wndTop, m_nOffset, 0, std::max(pFrame->StretchWidth(GetColumnsWidth(),m_wndRect.Width()),m_wndRect.Width()), m_headerRect.Height(), SWP_SHOWWINDOW);
					}
				}
			}
		}

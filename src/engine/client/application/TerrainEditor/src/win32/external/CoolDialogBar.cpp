...
CSize CCoolDialogBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
	...
	if (IsFloating())
	{
		...
		{
		case HTTOPLEFT:
			m_sizeFloat.cx = std::max(static_cast<int> (window_rect.right - cursor_pt.x), static_cast<int> (m_cMinWidth)) - m_cxBorder;
			m_sizeFloat.cy = std::max(static_cast<int> (window_rect.bottom - m_cCaptionSize - cursor_pt.y), static_cast<int> (m_cMinHeight)) - 1;
			m_pDockContext->m_rectFrameDragHorz.top = std::min(static_cast<int> (cursor_pt.y), static_cast<int> (window_rect.bottom - m_cCaptionSize - m_cMinHeight)) - m_cyBorder;
			m_pDockContext->m_rectFrameDragHorz.left = std::min(static_cast<int> (cursor_pt.x), static_cast<int> (window_rect.right - m_cMinWidth)) - 1;
			return m_sizeFloat;
			
		case HTTOPRIGHT:
			m_sizeFloat.cx = std::max(static_cast<int> (cursor_pt.x - window_rect.left),
				static_cast<int> (m_cMinWidth));
			m_sizeFloat.cy = std::max(static_cast<int> (window_rect.bottom - m_cCaptionSize - cursor_pt.y),m_cMinHeight) - 1;
			m_pDockContext->m_rectFrameDragHorz.top = std::min(static_cast<int> (cursor_pt.y), static_cast<int> (window_rect.bottom - m_cCaptionSize - m_cMinHeight)) - 
				m_cyBorder;
			return m_sizeFloat;
			
		case HTBOTTOMLEFT:
			m_sizeFloat.cx = std::max(static_cast<int> (window_rect.right - cursor_pt.x), static_cast<int> (m_cMinWidth)) - m_cxBorder;
			m_sizeFloat.cy = std::max(static_cast<int> (cursor_pt.y - window_rect.top - m_cCaptionSize), static_cast<int> (m_cMinHeight));
			m_pDockContext->m_rectFrameDragHorz.left = std::min(static_cast<int> (cursor_pt.x), static_cast<int> (window_rect.right - m_cMinWidth)) - 1;
			return m_sizeFloat;
			
		case HTBOTTOMRIGHT:
			m_sizeFloat.cx = std::max(static_cast<int> (cursor_pt.x - window_rect.left), static_cast<int> (m_cMinWidth));
			m_sizeFloat.cy = std::max(static_cast<int> (cursor_pt.y - window_rect.top - m_cCaptionSize), static_cast<int> (m_cMinHeight));
			return m_sizeFloat;
		}
	}

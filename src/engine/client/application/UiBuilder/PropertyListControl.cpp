...
namespace
{
        inline CString safeMakeCString(const char *value)
        {
...
CString PropertyListControlProperty::getNameForCopy() const
{
        return safeMakeCString(m_descriptor.m_name);
}

CString PropertyListControlProperty::getValueForCopy() const
{
        return CString(m_narrowValue.c_str());
}

CString PropertyListControlProperty::getDisplayName() const
{
        return safeMakeCString(m_descriptor.m_name);
}

void PropertyListControlProperty::toggleFavorite()
{
}
...
void PropertyListControl::addProperty(const Property &i_property)
{
        ...
        const CString displayName = i_property.getDisplayName();
        int item = InsertItem(GetItemCount(), displayName);
...
bool PropertyListControl::_contextMenu(int item, CPoint loc)
{
	...
	CMenu menu;
        ...
        const bool canCopyName = p->canCopyName();
        const bool canCopyValue = p->canCopyValue();
        const bool canCopyIdentifier = p->canCopyIdentifier();

        if (canCopyName || canCopyValue || canCopyIdentifier)
        {
                menu.AppendMenu(MF_SEPARATOR);

                if (canCopyName)
                        menu.AppendMenu(MF_STRING | MF_ENABLED, ID_COPY_PROPERTY_NAME, "Copy Name");

                if (canCopyValue)
                {
                        menu.AppendMenu(MF_STRING | MF_ENABLED, ID_COPY_PROPERTY_VALUE, "Copy Value");
                        menu.AppendMenu(MF_STRING | MF_ENABLED, ID_COPY_PROPERTY_DECLARATION, "Copy Name && Value");
                }

                if (canCopyIdentifier)
                        menu.AppendMenu(MF_STRING | MF_ENABLED, ID_COPY_PROPERTY_IDENTIFIER, "Copy Identifier");
        }

        if (p->supportsFavorites())
        {
                menu.AppendMenu(MF_SEPARATOR);
                const bool favorite = p->isFavorite();
                menu.AppendMenu(MF_STRING | MF_ENABLED, ID_TOGGLE_PROPERTY_FAVORITE, favorite ? "Remove Favorite" : "Add to Favorites");
        }
...
int PropertyListControl::getFirstSelectedIndex() const
{
        return GetNextItem(-1, LVNI_SELECTED);
}

PropertyListControl::Property *PropertyListControl::getPropertyAt(int index) const
{
        if (index < 0 || index >= GetItemCount())
                return 0;

        return reinterpret_cast<Property *>(GetItemData(index));
}

PropertyListControl::Property *PropertyListControl::getSelectedProperty() const
{
        const int selected = getFirstSelectedIndex();
        return getPropertyAt(selected);
}
...
BEGIN_MESSAGE_MAP(PropertyListControl, CListCtrl)
        ...
        ON_COMMAND(ID_COPY_PROPERTY_NAME, OnCopyPropertyName)
        ON_COMMAND(ID_COPY_PROPERTY_VALUE, OnCopyPropertyValue)
        ON_COMMAND(ID_COPY_PROPERTY_DECLARATION, OnCopyPropertyDeclaration)
        ON_COMMAND(ID_COPY_PROPERTY_IDENTIFIER, OnCopyPropertyIdentifier)
        ON_COMMAND(ID_TOGGLE_PROPERTY_FAVORITE, OnTogglePropertyFavorite)
END_MESSAGE_MAP()
...
void PropertyListControl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
        ...
        bool handled = false;
        const int selectedIndex = getFirstSelectedIndex();

        if (selectedIndex >= 0)
                m_contextItem = selectedIndex;

        if (!ctrl && !alt && nChar == VK_RETURN && selectedIndex >= 0)
        {
                CRect labelRect;
                if (GetItemRect(selectedIndex, &labelRect, LVIR_LABEL))
                {
                        CPoint screen = labelRect.TopLeft();
                        ClientToScreen(&screen);
                        _edit(selectedIndex, screen, false);
                        handled = true;
                }
        }
        else if (ctrl && !alt && !shift && (nChar == 'C') && selectedIndex >= 0)
        {
                OnCopyPropertyValue();
                handled = true;
        }
        else if (ctrl && shift && !alt && (nChar == 'C') && selectedIndex >= 0)
        {
                OnCopyPropertyName();
                handled = true;
        }
        ...
        {
                OnCopyPropertyDeclaration();
                handled = true;
        }
        else if (ctrl && !alt && (nChar == 'F') && selectedIndex >= 0)
        {
                OnTogglePropertyFavorite();
                handled = true;
        }
        else if (ctrl && shift && !alt && (nChar == 'I') && selectedIndex >= 0)
        {
                OnCopyPropertyIdentifier();
                handled = true;
        }
...
void PropertyListControl::OnCopyPropertyName()
{
        Property *p = getContextProperty();
        if (!p || !p->canCopyName())
                return;

        copyToClipboard(p->getNameForCopy());
}

void PropertyListControl::OnCopyPropertyValue()
{
        Property *p = getContextProperty();
        if (!p || !p->canCopyValue())
                return;

        copyToClipboard(p->getValueForCopy());
}

void PropertyListControl::OnCopyPropertyDeclaration()
{
        Property *p = getContextProperty();
        if (!p || !p->canCopyValue())
                return;
        ...
        CString declaration;
        declaration.Format("%s=%s", p->getNameForCopy(), p->getValueForCopy());
        copyToClipboard(declaration);
}

void PropertyListControl::OnCopyPropertyIdentifier()
{
        Property *p = getContextProperty();
        if (!p || !p->canCopyIdentifier())
                return;

        copyToClipboard(p->getIdentifierForCopy());
}

void PropertyListControl::OnTogglePropertyFavorite()
{
        Property *p = getContextProperty();
        if (!p || !p->supportsFavorites())
                return;

        p->toggleFavorite();
}

PropertyListControl::Property *PropertyListControl::getContextProperty()
{
        const int count = GetItemCount();
        Property *property = 0;

        if (m_contextItem >= 0 && m_contextItem < count)
                property = reinterpret_cast<Property *>(GetItemData(m_contextItem));
        ...
        if (!property)
        {
                ...
                if (selected >= 0)
                {
                        property = reinterpret_cast<Property *>(GetItemData(selected));
                        m_contextItem = selected;
                }
        }
...
void PropertyListControl::copyToClipboard(const CString &text) const
{
        ...
        if (global)
        {
                wchar_t *dest = static_cast<wchar_t *>(GlobalLock(global));
                if (dest)
                {
                        memcpy(dest, text.GetString(), bytes);
                        GlobalUnlock(global);
        ...
        if (global)
        {
                char *dest = static_cast<char *>(GlobalLock(global));
                if (dest)
                {
                        strcpy(dest, text.GetString());
                        GlobalUnlock(global);

...
bool ObjectEditor::upscaleSelectionsSmart()
{
	if (!hasSelections())
	{
		return false;
	...
	cancelDrag();
	return _edit_upscaleSelections(false);
}

// ==========================================================

bool ObjectEditor::upscaleSelectionsAuto()
{
        if (!hasSelections())
        {
                return false;
        ...
        cancelDrag();
        return _edit_upscaleSelections(true);
}

// ==========================================================

bool ObjectEditor::autoLayoutSelectionsResponsive(int preferredColumns, bool allowResize)
{
        if (!hasSelections())
        {
                return false;
        ...
        UIObjectVector selectedObjects;
        _getSelections(selectedObjects);
        ...
        std::vector<LayoutInfo> infos;
        infos.reserve(selectedObjects.size());

        UIBaseObject *sharedParent = 0;
        ...
        for (UIObjectVector::const_iterator it = selectedObjects.begin(); it != selectedObjects.end(); ++it)
        {
                ...
                LayoutInfo info;
                info.widget = widget;
                info.localRect = widget->GetRect();
                widget->GetWorldRect(info.worldRect);

                if (!sharedParent)
                {
                        sharedParent = widget->GetParent();
                }
                else if (sharedParent != widget->GetParent())
                {
                        sharedParent = 0;
                }

                minLeft = std::min(minLeft, info.localRect.left);
                minTop = std::min(minTop, info.localRect.top);
                maxRight = std::max(maxRight, info.localRect.right);
                maxBottom = std::max(maxBottom, info.localRect.bottom);
                maxWidth = std::max(maxWidth, info.localRect.Width());
                maxHeight = std::max(maxHeight, info.localRect.Height());
        ...
        UIWidget *parentWidget = UI_ASOBJECT(UIWidget, sharedParent);
        if (!parentWidget)
        {
                return false;
        }

        UIRect parentRect = parentWidget->GetRect();
        long availableWidth = parentRect.Width();
        long availableHeight = parentRect.Height();

        if (availableWidth <= 0)
        {
                availableWidth = std::max<long>(1, maxRight - minLeft);
        }

        if (availableHeight <= 0)
        {
                availableHeight = std::max<long>(1, maxBottom - minTop);
        }

        const int count = static_cast<int>(infos.size());
        int columns = (preferredColumns > 0) ? preferredColumns : static_cast<int>(std::ceil(std::sqrt(static_cast<float>(count))));
        columns = std::max(1, std::min(columns, count));
        int rows = (count + columns - 1) / columns;

        if (preferredColumns <= 0)
        {
                const float aspect = (availableHeight > 0) ? static_cast<float>(availableWidth) / static_cast<float>(availableHeight) : 1.0f;
                float bestScore = 1000000.0f;
                int bestColumns = columns;

                for (int candidate = 1; candidate <= count; ++candidate)
                {
                        const int candidateRows = (count + candidate - 1) / candidate;
                ...
                columns = std::max(1, std::min(bestColumns, count));
                rows = (count + columns - 1) / columns;
        }

        const UIPoint &gridSteps = getGridSteps();
        const bool snap = getSnapToGrid() && gridSteps.x > 0 && gridSteps.y > 0;

        const long paddingX = std::max<long>(snap ? gridSteps.x : 16L, 8L);
        const long paddingY = std::max<long>(snap ? gridSteps.y : 16L, 8L);

        long cellWidth = std::max<long>(maxWidth, (availableWidth - paddingX * (columns - 1)) / columns);
        long cellHeight = std::max<long>(maxHeight, (availableHeight - paddingY * (rows - 1)) / rows);
        ...
        const long contentWidth = columns * cellWidth + (columns - 1) * paddingX;
        const long contentHeight = rows * cellHeight + (rows - 1) * paddingY;

        long startX = parentRect.left;
        long startY = parentRect.top;

        if (availableWidth > contentWidth)
        {
                startX = parentRect.left + (availableWidth - contentWidth) / 2;
        }
        else
        {
                startX = std::max(parentRect.left, std::min(parentRect.right - cellWidth, minLeft));
        }

        if (availableHeight > contentHeight)
        {
                startY = parentRect.top + (availableHeight - contentHeight) / 2;
        }
        else
        {
                startY = std::max(parentRect.top, std::min(parentRect.bottom - cellHeight, minTop));
        }

        std::sort(infos.begin(), infos.end(), [](const LayoutInfo &lhs, const LayoutInfo &rhs)
        {
                if (lhs.worldRect.top == rhs.worldRect.top)
                {
                        return lhs.worldRect.left < rhs.worldRect.left;
                }
                return lhs.worldRect.top < rhs.worldRect.top;
        });
        ...
        std::vector<PendingRect> pending;
        ...
        for (int index = 0; index < count; ++index)
        {
                ...
                UIRect targetRect = infos[index].localRect;
                targetRect.left = startX + column * (cellWidth + paddingX);
                targetRect.top = startY + row * (cellHeight + paddingY);

                if (allowResize)
                {
                        targetRect.right = targetRect.left + cellWidth;
                        targetRect.bottom = targetRect.top + cellHeight;
                }
                else
                {
                        targetRect.right = targetRect.left + infos[index].localRect.Width();
                        targetRect.bottom = targetRect.top + infos[index].localRect.Height();
                }

                if (snap)
                {
                        UIPoint snapped(targetRect.left, targetRect.top);
                        gridClosest(snapped, snapped);
                        const long deltaX = snapped.x - targetRect.left;
                        const long deltaY = snapped.y - targetRect.top;
                        targetRect.left += deltaX;
                        targetRect.right += deltaX;
                        targetRect.top += deltaY;
                        targetRect.bottom += deltaY;
                }

                PendingRect pendingRect;
                pendingRect.widget = infos[index].widget;
                pendingRect.rect = targetRect;
                pending.push_back(pendingRect);
        }

        _beginEditOperation("Responsive Auto Layout");
        bool success = true;
        for (std::vector<PendingRect>::const_iterator it = pending.begin(); it != pending.end(); ++it)
        {
                if (!_eh_setObjectRect(*it->widget, it->rect))
                {
                        success = false;
                        break;
                }
...
bool ObjectEditor::centerSelectionInParent()
{
        ...
        UIWidget *widget = UI_ASOBJECT(UIWidget, anchor);
        if (!widget)
        {
                return false;
        }

        UIWidget *parentWidget = UI_ASOBJECT(UIWidget, widget->GetParent());
        if (!parentWidget)
        {
                return false;
        }
        ...
        UIRect parentRect = parentWidget->GetRect();
        UIRect localRect = widget->GetRect();

        const long width = localRect.Width();
        const long height = localRect.Height();

        const long targetLeft = parentRect.left + (parentRect.Width() - width) / 2;
        const long targetTop = parentRect.top + (parentRect.Height() - height) / 2;

        UIRect newRect(targetLeft, targetTop, targetLeft + width, targetTop + height);

        if (getSnapToGrid())
        {
                UIPoint snapped(newRect.left, newRect.top);
                gridClosest(snapped, snapped);
                const long deltaX = snapped.x - newRect.left;
                const long deltaY = snapped.y - newRect.top;
                newRect.left += deltaX;
        ...
        _beginEditOperation("Center Selection");
        const bool success = _eh_setObjectRect(*widget, newRect);
        _endEditOperation(success);
        return success;
}
...
bool ObjectEditor::_edit_upscaleSelections(bool centerExpansion)
{
	long recommendedWidth = 0;
	long recommendedHeight = 0;
	if (!_computeHdRecommendation(recommendedWidth, recommendedHeight))
	{
		return false;
	}
	...
	std::vector<PendingRect> pendingRects;
	pendingRects.reserve(selectedObjects.size());
	...
	for (UIObjectVector::const_iterator it = selectedObjects.begin(); it != selectedObjects.end(); ++it)
	{
		...
		UIRect localRect = widget->GetRect();
		const long currentWidth = localRect.Width();
		const long currentHeight = localRect.Height();

		long targetWidth = std::max(currentWidth, recommendedWidth);
		long targetHeight = std::max(currentHeight, recommendedHeight);
		...
		if (centerExpansion)
		{
			const long widthIncrease = targetWidth - currentWidth;
			const long heightIncrease = targetHeight - currentHeight;

			localRect.left -= widthIncrease / 2;
			localRect.top -= heightIncrease / 2;
		}

		localRect.right = localRect.left + targetWidth;
		localRect.bottom = localRect.top + targetHeight;

		PendingRect entry;
		entry.widget = widget;
		entry.rect = localRect;
		pendingRects.push_back(entry);
	}
	...
	_beginEditOperation(centerExpansion ? "Auto Upscale Selections" : "Smart Upscale Selections");
	bool success = true;

	for (std::vector<PendingRect>::const_iterator it = pendingRects.begin(); it != pendingRects.end(); ++it)
	{
		if (!_eh_setObjectRect(*it->widget, it->rect))
		{
			success = false;
...
bool ObjectEditor::_computeHdRecommendation(long &outWidth, long &outHeight) const
{
	if (!m_currentShowPage)
	{
		return false;
	...
	UIRect rootRect;
	m_currentShowPage->GetWorldRect(rootRect);
	...
	outWidth = computeHdTarget(64L, layoutScaleX);
	outHeight = computeHdTarget(48L, layoutScaleY);
	return true;
}
	...
	if (!incomingLinks.empty())
	{
		...
		_snprintf(
			...
			incomingLinks.size()
                ...
                MessageBox(0, buf, "UIBuilderV2", MB_OK) ;
		return false;
	}
	// --------------------------------------------------

...
bool MayaCustomizableShaderTemplateWriter::collectTextureCustomizations(const MFnDependencyNode &textureDependencyNode, const Tag &textureTag, SwappableTextureDataVector &swappableTextureDataVector)
{
	...
	for (unsigned i = 0; i < attachedTextureCount; ++i)
	{
		...
		const bool textureReferenceNameResult = MayaUtility::buildTextureReferenceName(mayaPathName, SetDirectoryCommand::getDirectoryString(TEXTURE_REFERENCE_DIR_INDEX), relativePathName);
		MESSENGER_REJECT(!textureReferenceNameResult, ("Attached texture node [%s] has unsupported path name [%s].\n", textureNode.name().asChar(), mayaPathName.c_str()));

...
bool MayaUtility::getTextureReferenceExtension(const std::string &sourceTextureName, std::string &textureReferenceExtension)
{
	std::string::size_type extensionPos = sourceTextureName.find_last_of('.');
	MESSENGER_REJECT(extensionPos == std::string::npos, ("texture file [%s] has no extension, expected .dds, .png, or .tga\n", sourceTextureName.c_str()));

	std::string extension = sourceTextureName.substr(extensionPos);
	for (std::string::iterator it = extension.begin(); it != extension.end(); ++it)
		*it = static_cast<char>(tolower(*it));

	if ((extension == ".dds") || (extension == ".png"))
	{
		textureReferenceExtension = extension;
		return true;
	}

	if (extension == ".tga")
	{
		textureReferenceExtension = ".dds";
		return true;
	}

	MESSENGER_REJECT(true, ("texture file [%s] has unsupported extension [%s], expected .dds, .png, or .tga\n", sourceTextureName.c_str(), extension.c_str()));
}

// ----------------------------------------------------------------------

bool MayaUtility::buildTextureReferenceName(const std::string &sourceTextureName, const std::string &textureReferenceDirectory, std::string &textureRelativeName)
{
	char textureBaseName[128];
	bool result = MayaUtility::stripBaseName(sourceTextureName.c_str(), textureBaseName, sizeof(textureBaseName), '/');
	MESSENGER_REJECT(!result, ("failed to strip base name from shader group texture filename \"%s\"\n", sourceTextureName.c_str()));

	std::string textureReferenceExtension;
	result = MayaUtility::getTextureReferenceExtension(sourceTextureName, textureReferenceExtension);
	MESSENGER_REJECT(!result, ("failed to determine texture reference extension for [%s]\n", sourceTextureName.c_str()));

	textureRelativeName = textureReferenceDirectory;
	textureRelativeName += textureBaseName;
	textureRelativeName += textureReferenceExtension;
	return true;
}
// ----------------------------------------------------------------------
/**

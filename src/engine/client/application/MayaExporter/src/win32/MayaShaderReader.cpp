	...
	)
{
	...
	if (textureNode.apiType() == MFn::kFileTexture)
	{
		...
		std::string textureRelativeName;
		result = MayaUtility::buildTextureReferenceName(filename, i_textureReferenceDirectory, textureRelativeName);
		MESSENGER_REJECT (!result, ("failed to build texture reference name from shader group texture filename \"%s\"\n", filename.c_str()));

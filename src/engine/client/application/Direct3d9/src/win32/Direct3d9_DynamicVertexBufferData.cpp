...
void Direct3d9_DynamicVertexBufferData::install()
{
	...
	int configuredSizeKb = ConfigDirect3d9::getDynamicVertexBufferSize();
	if (configuredSizeKb > 0)
	{
		ms_size = configuredSizeKb * 1024;
	}
	else
	{
		...
		else
		{
			...
			else if (videoMemory<=512)
			{
				ms_size=4096*1024;
			}
			else if (videoMemory<=2048)
			{
				ms_size=8192*1024;
			}
			else

...
void CuiCachedAvatarManager::saveCharacterList  ()
{
	...
	static const size_t buf_size = sizeof (buf);
        ...
        for (CharacterListMap::iterator it = s_characterListMap.begin (); it != s_characterListMap.end (); ++it)
        {
                ...
                bool wasSelectedFromLauncher = false;

                for (std::vector<std::string>::const_iterator launcherIt = launcherAvatarNames.begin (); launcherIt != launcherAvatarNames.end () && !wasSelectedFromLauncher; ++launcherIt)
                {
                        wasSelectedFromLauncher = !_strnicmp (launcherIt->c_str (), avatarName.c_str (), avatarName.size ());
                }

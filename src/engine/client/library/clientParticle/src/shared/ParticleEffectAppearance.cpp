...
void ParticleEffectAppearance::install()
{
        ...
        int const configuredMaxUserParticles = ConfigClientParticle::getParticleUserLimit();
        s_globalUserLimit = configuredMaxUserParticles;
        LocalMachineOptionManager::registerOption (s_globalUserLimit, "ParticleEffectAppearance", "globalUserLimit");

        if (s_globalUserLimit < 0)
                s_globalUserLimit = 0;
        else if (s_globalUserLimit > configuredMaxUserParticles)
                s_globalUserLimit = configuredMaxUserParticles;

...
{
	int s_maxQuadParticles = 4096;
	int s_maxMeshParticles = 1024;
	int s_maxParticleAttachments = 2048;

	float s_minGlobalLodDistanceMin = 0.0f;
	float s_minGlobalLodDistanceMax = 4096.0f;
	float s_minGlobalLodDistanceDefault = 0.0f;
	...
	float s_maxGlobalLodDistanceDefault = 1024.0f;
	float s_maxGlobalLodDistance = s_maxGlobalLodDistanceDefault;
	float s_swooshCullDistance = 512.0f;
...
void ConfigClientParticle::install()
{
	...
	// Raise the default user particle cap to match the higher pool sizes so
	// large effect sequences can play out without early termination.
	KEY_INT(particleUserLimit, 4096);
}

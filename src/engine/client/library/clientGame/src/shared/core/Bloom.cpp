...
void Bloom::install()
{
	...
	DebugFlags::registerFlag (ms_viewBlurredAlpha,  "ClientGame/Bloom", "viewBlurredAlpha");
        ...
        ms_standardDeviation = ConfigFile::getKeyFloat ("ClientGame/Bloom", "standardDeviation", 8.0f);
        ms_weightMultiplier = std::max(0.0f, ConfigFile::getKeyFloat ("ClientGame/Bloom",  "weightMultiplier", 2.35f));
        ms_softPower = std::max(0.1f, ConfigFile::getKeyFloat ("ClientGame/Bloom", "softPower", 1.1f));
        ms_glowScale = std::max(0.0f, ConfigFile::getKeyFloat ("ClientGame/Bloom", "glowScale", 1.25f));
...
void BloomNamespace::setBlurPixelShaderUserConstants(float xStep, float yStep)
{
        ...
        for (x = 0; x < 16; ++x)
        {
                ...
                sampleWeights[x] = result;
        }

        // soften the falloff to favor a broader, smoother glow
        float sum = 0.0f;
        for (x = 0; x < 16; ++x)
        {
                float const softened = std::pow(std::max(sampleWeights[x], 0.0f), ms_softPower);
                sampleWeights[x] = softened;
                sum += softened * 2.0f;
        }

        sum = std::max(sum, 0.0001f);
        float const normalizationScale = (ms_weightMultiplier * ms_glowScale) / sum;

        for (x = 0; x < 16; ++x)
        {
                float const result = sampleWeights[x] * normalizationScale;
                weights[x].r = result;
                weights[x].g = result;
                weights[x].b = result;
                weights[x].a = result;

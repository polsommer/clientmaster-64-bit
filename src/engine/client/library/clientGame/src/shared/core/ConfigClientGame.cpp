...
namespace ConfigClientGameNamespace
{
	...
	// Network Services
        ...
        int             ms_loginClientIndex;
        int             ms_resolvedLoginClientIndex;
	...
	bool            ms_useInteriorLayoutFiles;
        ...
        std::vector<std::string> ms_launcherAvatarNames;
        ...
        bool containsLauncherAvatarName (std::string const & name)
        {
                return std::find_if (ms_launcherAvatarNames.begin (), ms_launcherAvatarNames.end (),
                        [&name](std::string const & existing)
                        {
                                return _stricmp (existing.c_str (), name.c_str ()) == 0;
                        }) != ms_launcherAvatarNames.end ();
        }

        void addLauncherAvatarNameIfUnique (std::string name)
        {
                trimString (name);

                if (!name.empty () && !containsLauncherAvatarName (name))
                        ms_launcherAvatarNames.push_back (name);
        }

        void refreshLauncherAvatarNames ()
        {
                ms_launcherAvatarNames.clear ();

                std::string current;

                for (size_t i = 0; i <= ms_launcherAvatarName.size (); ++i)
                {
                        const char c = i < ms_launcherAvatarName.size () ? ms_launcherAvatarName [i] : '\0';

                        if (c == '\0' || c == ',' || c == ';')
                        {
                                addLauncherAvatarNameIfUnique (current);
                                current.clear ();
                        }
                        else
                        {
        ...
        void appendLauncherAvatarNamesFromNumberedConfig ()
        {
                for (int i = 1; i <= 6; ++i)
                {
                        std::string key = "launcherAvatarName" + intToString (i);

                        if (char const * const name = ConfigFile::getKeyString ("ClientGame", key.c_str (), 0, 0))
                                addLauncherAvatarNameIfUnique (name);
                }
        }

        void promoteLauncherAvatarNameForIndex (int index)
        {
                if (index <= 0)
                        return;

                std::string const key = "launcherAvatarName" + intToString (index);
                ...
                if (char const * const name = ConfigFile::getKeyString ("ClientGame", key.c_str (), 0, 0))
                {
                        ...
                        {
                                ms_launcherAvatarNames.erase (std::remove_if (ms_launcherAvatarNames.begin (), ms_launcherAvatarNames.end (),
                                        [&normalizedName](std::string const & existing)
                                        {
                                                return _stricmp (existing.c_str (), normalizedName.c_str ()) == 0;
                                        }),
                                        ms_launcherAvatarNames.end ());

                                ms_launcherAvatarNames.insert (ms_launcherAvatarNames.begin (), normalizedName);
                        }
                }
        }

        void appendLauncherAvatarNamesFromAiScenario ()
        {
                char const * const envScenarioPath = std::getenv ("SWG_AI_LOAD_SCENARIO");
                std::string scenarioPath = envScenarioPath && *envScenarioPath ? envScenarioPath : "plugin/ai_load_tester/Swg+ai.cfg";

                std::ifstream file (scenarioPath.c_str (), std::ios::in | std::ios::binary);
                if (!file)
                        return;
                ...
                for (;;)
                {
                        size_t const keyPos = contents.find ("\"character\"", searchOffset);
                        if (keyPos == std::string::npos)
                                break;

                        size_t const colonPos = contents.find (':', keyPos + 11);
                        if (colonPos == std::string::npos)
                                break;

                        size_t const valueBegin = contents.find ('"', colonPos);
                        if (valueBegin == std::string::npos)
                                break;

                        size_t const valueEnd = contents.find ('"', valueBegin + 1);
                        if (valueEnd == std::string::npos)
                                break;

                        std::string name (contents.begin () + valueBegin + 1, contents.begin () + valueEnd);
        ...
        void seedLoginCredentialsFromNumberedEntries ()
        {
                ms_resolvedLoginClientIndex = ms_loginClientIndex;

                if (ms_loginClientID && *ms_loginClientID)
                        return;

                auto const tryIndex = [](int const index) -> bool
                {
                        if (index <= 0)
                                return false;

                        std::string const idKey = "loginClientID" + intToString (index);
                        char const * const loginClientId = ConfigFile::getKeyString ("ClientGame", idKey.c_str (), 0, 0);

                        if (loginClientId && *loginClientId)
                        {
                                std::string const passwordKey = "loginClientPassword" + intToString (index);
                                ms_loginClientID = loginClientId;
                                ms_loginClientPassword = ConfigFile::getKeyString ("ClientGame", passwordKey.c_str (), "", 0);
                                ms_resolvedLoginClientIndex = index;
                                promoteLauncherAvatarNameForIndex (index);
                                return true;
                        }
                ...
                // Respect an explicitly requested login slot when present
                if (tryIndex (ms_loginClientIndex))
                        return;

                for (int i = 1; i <= 6; ++i)
                {
                        if (i == ms_loginClientIndex)
                                continue;
...
void ConfigClientGame::install(void)
{
	...
	KEY_BOOL   (allowDefaultTemplateParams,    false);
        ...
        KEY_STRING(loginServerAddress, "login.swgplus.com");
        KEY_INT    (loginServerPort,               44453);
        KEY_INT    (loginClientIndex,              0);
        KEY_STRING (loginClientID,                 "");
        KEY_STRING (loginClientPassword,           "");

        seedLoginCredentialsFromNumberedEntries ();

        //-- Allow numbered login server entries to seed the default login server
        //-- selection for flows that read directly from ConfigClientGame instead of
        //-- parsing the list themselves.
        if (char const *loginServerAddress0 = ConfigFile::getKeyString("ClientGame", "loginServerAddress0", 0, 0))
        {
                uint16 const loginServerPort0 = static_cast<uint16>(ConfigFile::getKeyInt("ClientGame", "loginServerPort0", 0, 0));

                if (loginServerPort0 != 0)
                {
                        ms_loginServerAddress = loginServerAddress0;
                        ms_loginServerPort = loginServerPort0;
                }
        }

	KEY_BOOL   (autoConnectToLoginServer,      false);
	KEY_STRING (centralServerName,             "swg");
	...
	KEY_FLOAT  (freeChaseCameraZoomSpeed,  0.2f);
        ...
        refreshLauncherAvatarNames ();
        appendLauncherAvatarNamesFromNumberedConfig ();
        appendLauncherAvatarNamesFromAiScenario ();
        promoteLauncherAvatarNameForIndex (ms_resolvedLoginClientIndex);
...
const std::vector<std::string> & ConfigClientGame::getLauncherAvatarNames ()
{
        return ms_launcherAvatarNames;
}

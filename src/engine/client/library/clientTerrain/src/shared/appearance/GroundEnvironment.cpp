...
namespace GroundEnvironmentNamespace
{
	...
	bool ms_forceFallbackLighting;
        ...
        const float ms_waterBaseReflectivity = Fresnel::DefaultWaterReflectivity;
	...
	bool s_installed = false;
        ...
        void clampColor01(VectorArgb &c)
        {
...
void GroundEnvironment::apply (float const t)
{
	...
	// lighting colors
        ...
        bool const isInterior = (m_referenceObject && !m_referenceObject->isInWorldCell ());

        float mainDiffuseColorScale  = ::linearInterpolate (m_previousEnvironmentBlock->getMainDiffuseColorScale () [m_currentColorIndex], m_currentEnvironmentBlock->getMainDiffuseColorScale () [m_currentColorIndex], t);
        float mainSpecularColorScale = ::linearInterpolate (m_previousEnvironmentBlock->getMainSpecularColorScale () [m_currentColorIndex], m_currentEnvironmentBlock->getMainSpecularColorScale () [m_currentColorIndex], t);
        ...
        if (isInterior)
        {
                float const interiorLightBoost = ConfigClientTerrain::getInteriorLightBoost ();
                float const interiorAmbientDampen = ConfigClientTerrain::getInteriorAmbientDampen ();

                mainDiffuseColorScale *= interiorLightBoost;
                mainSpecularColorScale *= interiorLightBoost;
                fillColorScale *= interiorLightBoost;
                bounceColorScale *= interiorLightBoost;

                ambientColor = ambientColor * interiorAmbientDampen;
                mainDiffuseColor = mainDiffuseColor * interiorLightBoost;
                mainSpecularColor = mainSpecularColor * interiorLightBoost;
                fillColor = fillColor * interiorLightBoost;
                bounceColor = bounceColor * interiorLightBoost;

                clampColor01 (ambientColor);
                clampColor01 (mainDiffuseColor);
                clampColor01 (mainSpecularColor);
                clampColor01 (fillColor);
                clampColor01 (bounceColor);
        }
	...
	// ----------------------------------------------------------------------
        ...
        if (isInterior && m_currentInteriorEnvironmentBlock)
        {
                float const interiorFogDensity = m_currentInteriorEnvironmentBlock->getFogDensity () * ConfigClientTerrain::getInteriorFogDensityScale ();
                m_fogColor = VectorArgb::linearInterpolate (m_fogColor, m_currentInteriorEnvironmentBlock->getFogColor (), 0.6f);
                m_fogDensity = ::linearInterpolate (m_fogDensity, interiorFogDensity, 0.6f);
        }
	...
	if (texture)
	{
		...
		//-- scale the color ramp
		{
			...
			for (i = 0; i < ms_skyEnvironmentTextureSize; ++i)
                ...
                //-- create +y
                {
                        ...
                        m_skyEnvironmentMap->lock (lockData);
                                ...
                                const PackedRgb fresnelColor = Fresnel::applyToColor(colorRamp[0], 1.0f, ms_waterBaseReflectivity);
                                ...
                                for (y = 0; y < ms_skyEnvironmentTextureSize; ++y)
                                {
                                        ...
                                        {
                                                *destination++ = fresnelColor.b;
                                                *destination++ = fresnelColor.g;
                                                *destination++ = fresnelColor.r;
                                                *destination++ = 255;
                                        }
                                }
                ...
                //-- create -y
                {
                        ...
                        m_skyEnvironmentMap->lock (lockData);
                                ...
                                const PackedRgb fresnelColor = Fresnel::applyToColor(colorRamp[ms_skyEnvironmentTextureSize - 1], 0.0f, ms_waterBaseReflectivity);
			...
			m_skyEnvironmentMap->unlock (lockData);
                ...
                //-- create +x, -x, +z, -z
                {
                        const int lastColorIndex = ms_skyEnvironmentTextureSize - 1;
                        const int halfSize = ms_skyEnvironmentTextureSize / 2;
                        const float halfSizeF = halfSize > 0 ? static_cast<float>(halfSize) : 1.0f;
                        const float invLastIndex = lastColorIndex > 0 ? 1.0f / static_cast<float>(lastColorIndex) : 0.0f;

                        for (int i = 0; i < CF_none; ++i)
                        {
                                ...
                                m_skyEnvironmentMap->lock (lockData);
                                        ...
                                        for (int y = 0; y < ms_skyEnvironmentTextureSize; ++y)
                                        {
                                                PackedRgb sourceColor;
                                                if (y < halfSize)
                                                {
                                                        float rampSample = halfSizeF > 0.0f ? static_cast<float>(y) / halfSizeF : 0.0f;
                                                        rampSample = clampUnit(rampSample);
                                                        const float rampIndexFloat = rampSample * rampSample * static_cast<float>(lastColorIndex);
                                                        int rampIndex = static_cast<int>(rampIndexFloat);
                                                        if (rampIndex < 0)
                                                                rampIndex = 0;
                                                        else if (rampIndex > lastColorIndex)
                                                                rampIndex = lastColorIndex;
                                                        sourceColor = colorRamp[rampIndex];
                                                }
                                                else
                                                {
                                                        sourceColor = colorRamp[lastColorIndex];
                                                }

                                                float cosTheta = invLastIndex > 0.0f ? 1.0f - static_cast<float>(y) * invLastIndex : 1.0f;
                                                cosTheta = clampUnit(cosTheta);

                                                const PackedRgb fresnelColor = Fresnel::applyToColor(sourceColor, cosTheta, ms_waterBaseReflectivity);

                                                uint8* destination = pixelData + (y * lockData.getPitch ());

                                                for (int x = 0; x < ms_skyEnvironmentTextureSize; ++x)
                                                {
                                                        *destination++ = fresnelColor.b;
                                                        *destination++ = fresnelColor.g;
                                                        *destination++ = fresnelColor.r;

...
protected:
	...
	VertexBufferFormat _getVertexFormat() const;
        ...
        void _fillGraphicsBuffersFFP(VertexBufferWriteIterator i_vv, Index *i_ii, Vector normal, const Vector &cameraPosition) const;
...
private:
	...
	mutable unsigned            m_renderQuadrants;
        ...
        mutable Vector m_lastCameraPosition;
};
...
:	LocalShaderPrimitive(appearance, mapWidthInMeters, shaderTemplateName, shaderSize, height, clipRegion),
	...
	m_farPlane(0),
        m_resolution(0),
        m_lastCameraPosition(Vector::zero)
{
}
...
void ClientGlobalWaterManager2::StaticShaderPrimitive::_buildGraphicsBuffers() const
{
	...
	Index *ii = m_indexBuffer->begin();
        ...
        else
        {
                ...
                _fillGraphicsBuffersFFP(vv, ii, normal, m_lastCameraPosition);
        }
...
void ClientGlobalWaterManager2::StaticShaderPrimitive::prepareToDraw () const
{
	...
	if (m_renderQuadrants==0)
	{
        ...
        m_lastCameraPosition = camera.getPosition_w();
...
void ClientGlobalWaterManager2::DynamicShaderPrimitive::prepareToDraw () const
{
	...
	Index *ii = m_indexBuffer.begin();
        ...
        {
                const Vector normal = (above) ? Vector(0,1,0) : Vector(0,-1,0);
                _fillGraphicsBuffersFFP(vv, ii, normal, camera.getPosition_w());
        }
																					  ...
																					  ) const
{
	...
	*/
        const bool enableHighQualityTessellation = Graphics::supportsHighQualityTessellation() && Graphics::isHighQualityTessellationEnabled();

        // Maintain a minimum altitude so the tessellation math remains stable. On DX9 with
        // high-quality tessellation enabled we let the mesh react to the viewer height so
        // triangle density falls off naturally with altitude. Older or lower-quality paths
        // retain the fixed distance to limit overdraw on less capable hardware.
        const float altitude = enableHighQualityTessellation ? std::max(1.0f, std::fabs(referencePosition_w.y - m_height)) : 2.0f;
        const float altitudeSquared = altitude * altitude;
...
void ClientGlobalWaterManager2::LocalShaderPrimitive::_fillGraphicsBuffersFFP(VertexBufferWriteIterator i_vv, Index *const i_ii, Vector normal, const Vector &cameraPosition) const
{
		...
		////////////////////////////////////
                ...
                const float magnitudeSquared = toCamera.magnitudeSquared();
                float cosTheta = 1.0f;
                if (magnitudeSquared > 0.0f)
                {
                        toCamera.normalize();
                        cosTheta = std::fabs(normalUnit.dot(toCamera));
                }

                const float reflectance = Fresnel::computeReflectance(cosTheta, Fresnel::DefaultWaterReflectivity);
                float reflectanceClamped = reflectance;
                if (reflectanceClamped < 0.0f)
                        reflectanceClamped = 0.0f;
                else if (reflectanceClamped > 1.0f)
                        reflectanceClamped = 1.0f;
                const uint8 fresnelComponent = static_cast<uint8>(reflectanceClamped * 255.0f + 0.5f);
                vv.setColor0(PackedArgb(255, fresnelComponent, fresnelComponent, fresnelComponent));
        }
        {
                uint i;

...
#include "clientGraphics/TessellationOptionTags.h"
...
namespace GraphicsNamespace
{
	...
	void  defaultTranslatePointFromGameToScreen(int &x, int &y);
        ...
        void  updateTessellationOptionTags();
...
bool                                      ms_supportsTessellation;
bool                                      ms_highQualityTessellationEnabled;
...
ms_highQualityTessellationEnabled = ConfigClientGraphics::getEnableHighQualityTessellation();
...
LocalMachineOptionManager::registerOption(ms_highQualityTessellationEnabled, "ClientGraphics", "highQualityTessellation");
	...
	ms_supportsTessellation = (ms_api->supportsTessellation && ms_api->supportsTessellation());

	// Some DX9 renderers expose tessellation toggling without an explicit capability query.
	// If the toggle hook exists, assume support on the DX9 path so the feature can be
	// enabled at runtime instead of being silently disabled by the missing query.
	if (!ms_supportsTessellation && ms_api->setTessellationEnabled && ms_rasterMajor >= 5)
	{
		ms_supportsTessellation = true;
	}
...
setHighQualityTessellationEnabled(ms_highQualityTessellationEnabled);
...
void GraphicsNamespace::updateTessellationOptionTags()
{
        const bool tessellationActive = ms_supportsTessellation && ms_highQualityTessellationEnabled;
        GraphicsOptionTags::set(TessellationOptionTags::getTerrainTag(), tessellationActive);
        GraphicsOptionTags::set(TessellationOptionTags::getCharacterTag(), tessellationActive);
}
...
bool Graphics::isTextureSizeSupported(int width, int height)
{
	if (width <= 0 || height <= 0)
		return false;

	if (ms_api == NULL || ms_api->isTextureSizeSupported == NULL)
		return width <= ms_currentRenderTargetMaxWidth && height <= ms_currentRenderTargetMaxHeight;

	return ms_api->isTextureSizeSupported(width, height);
}

// ----------------------------------------------------------------------

bool Graphics::requiresPowerOfTwoTextures()
{
	if (ms_api == NULL || ms_api->requiresPowerOfTwoTextures == NULL)
		return false;

	return ms_api->requiresPowerOfTwoTextures();
}
...
bool Graphics::supportsHighQualityTessellation()
{
	return ms_supportsTessellation;
}

// ----------------------------------------------------------------------

bool Graphics::isHighQualityTessellationEnabled()
{
	return ms_highQualityTessellationEnabled;
}

// ----------------------------------------------------------------------

void Graphics::setHighQualityTessellationEnabled(bool enabled)
{
        ms_highQualityTessellationEnabled = enabled;

        if (!ms_supportsTessellation)
        {
                updateTessellationOptionTags();
                DEBUG_REPORT_LOG(ms_highQualityTessellationEnabled, ("High quality tessellation requested but not supported by backend; falling back.\n"));
                return;
        }

        if (ms_api->setTessellationEnabled)
                ms_api->setTessellationEnabled(ms_highQualityTessellationEnabled);

...
namespace
{
	bool meetsHighQualityRequirements()
	{
		if (!ConfigClientGraphics::getEnableHighQualityPreset())
			return false;

                const int requiredCapability = ShaderCapability(
                        ConfigClientGraphics::getHighQualityMinimumShaderCapabilityMajor(),
                        ConfigClientGraphics::getHighQualityMinimumShaderCapabilityMinor());

		if (Graphics::getShaderCapability() < requiredCapability)
			return false;

		if (Graphics::getVideoMemoryInMegabytes() < ConfigClientGraphics::getHighQualityMinimumVideoMemoryMB())
			return false;

		if (MemoryManager::getLimit() < ConfigClientGraphics::getHighQualityMinimumSystemMemoryMB())
			return false;

		return true;
	}

        void applyHighQualityGraphicsPreset()
        {
                if (!meetsHighQualityRequirements())
                        return;

		GraphicsOptionTags::set(TAG(D,O,T,3), true);
		GraphicsOptionTags::set(TAG(P,O,S,T), true);
		GraphicsOptionTags::set(TAG(H,E,A,T), true);

		if (ConfigClientGraphics::getHighQualityEnableCinematicEffects())
		{
			PostProcessingEffectsManager::applyHighQualityPreset(ConfigClientGraphics::getHighQualityCinematicStrength());
		}

                if (ConfigClientGraphics::getHighQualityEnableAntialiasing())
                {
                        PostProcessingEffectsManager::setAntialiasEnabled(true);
                }
        }

        void configureTessellationOptionTags()
        {
                const bool tessellationSupported = Graphics::supportsHighQualityTessellation();
                const bool tessellationEnabled = tessellationSupported && Graphics::isHighQualityTessellationEnabled();

                GraphicsOptionTags::set(TessellationOptionTags::getTerrainTag(), tessellationEnabled);
                GraphicsOptionTags::set(TessellationOptionTags::getCharacterTag(), tessellationEnabled);

                if (!tessellationSupported && ConfigSharedFoundation::getVerboseHardwareLogging())
                        REPORT_LOG(true, ("Tessellation disabled: backend does not report support.\n"));
        }
}
	...
	if (data.use3dSystem)
	{
		...
		if (Graphics::getShaderCapability() < ShaderCapability(2,0) || Graphics::getVideoMemoryInMegabytes() < 100 || MemoryManager::getLimit() < 260)
		{
			...
			REPORT_LOG(ConfigSharedFoundation::getVerboseHardwareLogging(), ("Disabling Post Processing effects\n"));
                ...
                applyHighQualityGraphicsPreset();
                configureTessellationOptionTags();

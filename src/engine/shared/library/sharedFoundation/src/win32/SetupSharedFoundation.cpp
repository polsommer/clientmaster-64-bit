...
LONG __stdcall SetupSharedFoundationNamespace::MyUnhandledExceptionFilter(LPEXCEPTION_POINTERS exceptionPointers)
{
	...
	if (!(exceptionPointers && exceptionPointers->ExceptionRecord))
	{
		OutputDebugString("MyUnhandledExceptionFilter called without valid EXCEPTION_POINTERS\n");
	}
	...
	if (ms_writeMiniDumps && !ms_alreadyWroteMiniDump)
	{
		...
		{
			wroteMiniDumpFile = false;
			OutputDebugString("Minidump skipped due to invalid EXCEPTION_POINTERS/CONTEXT\n");
		}

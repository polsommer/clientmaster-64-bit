...
namespace TreeFileNamespace
{
	...
	public:
        ...
        bool computeAlternateTreePath(char const *fileName, char *alternatePath, size_t alternatePathSize)
        {
                if (!fileName || !alternatePath || alternatePathSize == 0)
                        return false;

                std::string const originalPath(fileName);
                std::string::size_type const dotPos = originalPath.find_last_of('.');
                if (dotPos == std::string::npos)
                        return false;

                std::string const extension = originalPath.substr(dotPos);
                std::string alternate;

                if (_stricmp(extension.c_str(), ".tre") == 0)
                {
                        std::string const basePath(originalPath.substr(0, dotPos));
                        alternate = basePath;
                        alternate.append(".tresx");
                        if (!FileStreamer::exists(alternate.c_str()))
                        {
                                alternate = basePath;
                                alternate.append(".tres");
                        }
                }
                else if (_stricmp(extension.c_str(), ".tresx") == 0)
                {
                        alternate = originalPath.substr(0, dotPos);
                        alternate.append(".tre");
                }
                else if (_stricmp(extension.c_str(), ".tres") == 0)
                {
                        alternate = originalPath.substr(0, dotPos);
                        alternate.append(".tre");
                }
                ...
                if (alternate.size() + 1 > alternatePathSize)
                        return false;

                strcpy(alternatePath, alternate.c_str());
                return true;
        }

        bool computeTreePathFromToc(char const *fileName, char *treePath, size_t treePathSize)
        {
                if (!fileName || !treePath || treePathSize == 0)
                        return false;
                ...
                std::string treeFile(originalPath.substr(0, dotPos));
                treeFile.append(".tre");

                if (treeFile.size() + 1 > treePathSize)
                        return false;

                strcpy(treePath, treeFile.c_str());
                return true;
        }
...
void TreeFile::addSearchTree(const char *fileName, int priority)
{
        ...
        char alternatePath[Os::MAX_PATH_LENGTH];
        if (computeAlternateTreePath(fileName, alternatePath, sizeof(alternatePath)) && FileStreamer::exists(alternatePath))
        {
                WARNING(true, ("TreeFile::addSearchTree - [%s] not found, using [%s] instead", fileName, alternatePath));
                addSearchNode(new SearchTree(priority, alternatePath));
                return;
        }
...
void TreeFile::addSearchTOC(const char *fileName, int priority)
{
	...
	{
		char treePath[Os::MAX_PATH_LENGTH];
		if (computeTreePathFromToc(fileName, treePath, sizeof(treePath)))
		{
			if (FileStreamer::exists(treePath))
			{
				WARNING(true, ("TreeFile::addSearchTOC - using tree [%s] for missing TOC [%s]", treePath, fileName));
				addSearchNode(new SearchTree(priority, treePath));
				return;
			}

			char alternatePath[Os::MAX_PATH_LENGTH];
			if (computeAlternateTreePath(treePath, alternatePath, sizeof(alternatePath)) && FileStreamer::exists(alternatePath))
			{
				WARNING(true, ("TreeFile::addSearchTOC - using tree [%s] for missing TOC [%s]", alternatePath, fileName));
				addSearchNode(new SearchTree(priority, alternatePath));
				return;
			}
		}

		if (ConfigSharedFile::getAllowMissingTOC())
		{
...
{
        if (SearchTree::validate(fileName))
                return true;

        char alternatePath[Os::MAX_PATH_LENGTH];
        if (computeAlternateTreePath(fileName, alternatePath, sizeof(alternatePath)))
                return SearchTree::validate(alternatePath);

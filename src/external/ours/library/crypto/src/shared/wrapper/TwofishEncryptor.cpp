...
std::vector<unsigned char> TwofishEncryptor::s_lastKey;
...
void TwofishEncryptor::setKey(const unsigned char * const keyData, const unsigned int keyLength)
{
	...
	s_lastKey.assign(keyData, keyData + keyLength);
}

//-----------------------------------------------------------------------

void TwofishEncryptor::getLastKey(std::vector<unsigned char> & keyData)
{
	keyData = s_lastKey;
}
...
unsigned int TwofishEncryptor::getLastKeyLength()
{
	return static_cast<unsigned int>(s_lastKey.size());
}

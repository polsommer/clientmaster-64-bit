...
// Modern toolchains (including the Visual Studio 2013 build used by the
// contemporary client) provide a fully functional standard namespace.  The
// legacy Crypto++ configuration supported compilers without namespace support
// by defining `NO_NAMESPACE`, which in turn replaced `std::` references with
// the global namespace via `#define std`.  That substitution is harmful for
// modern standard headers such as <type_traits>, where removing the namespace
// qualifier breaks every declaration.  Automatically disable the legacy code
// path whenever a conforming namespace implementation is available.

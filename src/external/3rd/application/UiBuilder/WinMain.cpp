...
namespace
{
        ...
        void ResetContentScan(ContentScan &scan)
        {
                scan.widgetCount = 0;
                scan.textCount = 0;
                scan.imageCount = 0;
                scan.unnamedCount = 0;
                scan.emptyTextCount = 0;
                scan.unstyledTextCount = 0;
                scan.imagelessCount = 0;
        }

        void CollectContentScan(ContentScan &scan, UIBaseObject *root)
        {
                ResetContentScan(scan);
        ...
        void AppendContentSummary(const ContentScan &scan, std::ostringstream &stream)
        {
                if (scan.widgetCount <= 0)
                        return;

                stream << "  " << scan.widgetCount << " widgets, " << scan.textCount << " text, " << scan.imageCount << " images";

                if (scan.unnamedCount > 0 || scan.emptyTextCount > 0 || scan.unstyledTextCount > 0 || scan.imagelessCount > 0)
                {
                        stream << " |";

                        if (scan.unnamedCount > 0)
                                stream << ' ' << scan.unnamedCount << " unnamed";

                        if (scan.emptyTextCount > 0)
                                stream << ' ' << scan.emptyTextCount << " empty text";

                        if (scan.unstyledTextCount > 0)
                                stream << ' ' << scan.unstyledTextCount << " unstyled";

                        if (scan.imagelessCount > 0)
                                stream << ' ' << scan.imagelessCount << " imageless";
                }
        }
}
...
void SetMainWindowTitle( void )
{
	if( gUIWindow )
	{
		...
		{
			ContentScan scan;
			CollectContentScan( scan, gCurrentlySelectedPage );
			AppendContentSummary( scan, title );
		}

		const std::string composed = title.str();
		strncpy( buffer, composed.c_str(), sizeof( buffer ) - 1 );
...
BOOL CALLBACK MainWindowProc( HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	...
	case WM_INITDIALOG:
		{	
			...
			UiBuilder::ModernTheme::applyToDialog(hwndDlg);
			...
			SendMessage( gObjectTree, CCM_SETVERSION, 6, 0 );
			TreeView_SetExtendedStyle( gObjectTree, TVS_EX_DOUBLEBUFFER, TVS_EX_DOUBLEBUFFER );
			TreeView_SetBkColor( gObjectTree, UiBuilder::ModernTheme::panelColor() );
			TreeView_SetTextColor( gObjectTree, UiBuilder::ModernTheme::textColor() );
			TreeView_SetLineColor( gObjectTree, UiBuilder::ModernTheme::subtleTextColor() );
			...
			static const UINT sObjectToolbarIds[] =
			{
				IDC_TOP,
				IDC_UP,
				IDC_DOWN,
			...
			static const UINT sNavigationButtons[] =
			{
				IDC_BUTTON_BACK,
				IDC_BUTTON_FORWARD,
				IDC_BUTTON_UP
			...
			static const UINT sAccentLabels[] =
			{
				IDC_OBJECTSTRUCTURE_LABEL,
				IDC_OBJECTPROPERTIES_LABEL,
				IDC_PERFORMANCESTATS
			};

			UiBuilder::ModernTheme::styleToolbar( hwndDlg, sObjectToolbarIds, sizeof( sObjectToolbarIds ) / sizeof( sObjectToolbarIds[0] ) );
			UiBuilder::ModernTheme::styleToolbar( hwndDlg, sNavigationButtons, sizeof( sNavigationButtons ) / sizeof( sNavigationButtons[0] ) );
			UiBuilder::ModernTheme::applyAccentToControls( hwndDlg, sAccentLabels, sizeof( sAccentLabels ) / sizeof( sAccentLabels[0] ) );
		...
		case WM_ERASEBKGND:
			UiBuilder::ModernTheme::paintDialog( hwndDlg, reinterpret_cast<HDC>( wParam ) );
			return TRUE;
		...
		case WM_CTLCOLORBTN:
		{
			HBRUSH brush = UiBuilder::ModernTheme::handleCtlColor( reinterpret_cast<HWND>( lParam ), reinterpret_cast<HDC>( wParam ), uMsg );

	...
	case WM_INITDIALOG:
		{
			...
			static const UINT s_previewToolbarIds[] =
			{
				IDC_PLAY,
				IDC_REFRESH,
				IDC_DRAWCURSOR,
			...
			UiBuilder::ModernTheme::styleToolbar(hwndDlg, s_previewToolbarIds, sizeof(s_previewToolbarIds) / sizeof(s_previewToolbarIds[0]));
	...
	case WM_ERASEBKGND:
		{
			...
			RECT toolbarRect = { 0, 0, 0, 0 };
			RECT clientRect;
			GetClientRect(hwndDlg, &clientRect);

			HWND tabControl = GetDlgItem(hwndDlg, IDC_ROOTPAGE);

			if (tabControl)
			{
				RECT tabRect;
				GetWindowRect(tabControl, &tabRect);
				MapWindowPoints(NULL, hwndDlg, reinterpret_cast<LPPOINT>(&tabRect), 2);

				toolbarRect.left   = 0;
				toolbarRect.top    = 0;
				toolbarRect.right  = clientRect.right;
				toolbarRect.bottom = tabRect.top - 4;
			...
			else
			{
				...
				toolbarRect.top    = 0;
				toolbarRect.right  = clientRect.right;
				toolbarRect.bottom = 60;
			}

			UiBuilder::ModernTheme::paintToolbar(hwndDlg, reinterpret_cast<HDC>(wParam), toolbarRect);
			return TRUE;
		}

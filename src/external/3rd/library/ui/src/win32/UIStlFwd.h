...
// gcc requires dependent names to be prefixed with "typename" while the
// legacy MSVC6 compiler rejects it.  Modern MSVC releases (including the
// one used to build the client) also require the keyword, so only omit it
// for that specific compiler version.
#if defined(_MSC_VER) && (_MSC_VER <= 1200)
#define UI_FORWARD_TYPENAME
#else
#define UI_FORWARD_TYPENAME typename
#endif
...
template <class _Tp, class _Container = UI_FORWARD_TYPENAME ui_stdvector<_Tp>::fwd, class _Compare = std::less<typename _Container::value_type> > struct ui_stdpriority_queue
{
        typedef std::priority_queue<_Tp, _Container, _Compare> fwd;
};

template <class _Tp, class _Sequence = UI_FORWARD_TYPENAME ui_stddeque<_Tp>::fwd > struct ui_stdqueue
{
        typedef std::queue<_Tp, _Sequence> fwd;
};

template <class _Tp, class _Sequence = UI_FORWARD_TYPENAME ui_stddeque<_Tp>::fwd > struct ui_stdstack
{

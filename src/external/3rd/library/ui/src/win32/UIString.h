...
namespace UIUnicode
{
	typedef Unicode::unicode_char_t unicode_char_t;
        ...
        {
                return _wcsicmp (reinterpret_cast<const wchar_t *>(s1.c_str ()), reinterpret_cast<const wchar_t *>(s2.c_str ()));
        }

        inline int nicmp (const UIString & s1, const UIString & s2, size_t len)
        {
                return _wcsnicmp (reinterpret_cast<const wchar_t *>(s1.c_str ()), reinterpret_cast<const wchar_t *>(s2.c_str ()), len);
        }

...
namespace SwgCuiLoadingGroundNamespace
{
	...
	std::string resolveTextureResourceName(std::string const & logicalTexturePath)
	{
		static char const * const s_extensions[] = { ".dds", ".png", ".pgn" };

		for (size_t i = 0; i < sizeof(s_extensions) / sizeof(s_extensions[0]); ++i)
		{
			char const * const extension = s_extensions[i];
			if (TreeFile::exists((texturePre + logicalTexturePath + extension).c_str()))
			{
				if (i == 0)
					return logicalTexturePath;

				return logicalTexturePath + extension;
			}
		}
...
)
{
	...
	else
	{
		...
		std::string const resolvedFlagImageRes = resolveTextureResourceName(flagImageRes);
		if (resolvedFlagImageRes.empty())
		{
			flagImageRes = defaultFlag;
		}
		else
		{
			flagImageRes = resolvedFlagImageRes;
		}
		...
		std::string const resolvedPlanetImageRes = resolveTextureResourceName(planetImageRes);
		if (resolvedPlanetImageRes.empty())
		{
			planetImageRes = defaultPlanet;
		}
		else
		{
			planetImageRes = resolvedPlanetImageRes;
		}
	...
	std::string const resolvedScreenshot = resolveTextureResourceName(localPicture);
	if(resolvedScreenshot.empty())

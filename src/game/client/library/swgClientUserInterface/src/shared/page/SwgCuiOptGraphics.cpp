...
namespace SwgCuiOptGraphicsNamespace
{
	...
	bool getHighQualityTessellationDefault()
	{
		return ConfigClientGraphics::getEnableHighQualityTessellation();
	}
...
m_sliderGamma      (0)
{
	...
	if(Graphics::getShaderCapability() < ShaderCapability(1, 1))
	{
        ...
        if (getCodeDataObject (TUICheckbox, checkbox, "checkHighQualityTessellation", true))
        {
                registerCheckbox (*checkbox, Graphics::setHighQualityTessellationEnabled, Graphics::isHighQualityTessellationEnabled, SwgCuiOptGraphicsNamespace::getHighQualityTessellationDefault);
                if (!Graphics::supportsHighQualityTessellation())
                {
	...
	registerCheckbox (*checkbox, CuiPreferences::setDisableAnimationPriorities, CuiPreferences::getDisableAnimationPriorities, ConfigClientAnimation::getEnableCombatTrumping);
        ...
        int const maxUserParticles = ConfigClientParticle::getParticleUserLimit();
        registerSlider (*slider, setParticleUserLimit, getParticleUserLimit, ConfigClientParticle::getParticleUserLimit, 0, maxUserParticles);

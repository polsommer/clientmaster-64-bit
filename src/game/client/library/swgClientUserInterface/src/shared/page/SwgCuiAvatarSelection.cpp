...
void SwgCuiAvatarSelection::addAvatar (const CuiLoginManagerAvatarInfo & avatarInfo)
{
	...
	model->AppendCell (3, narrowName.c_str (), statusDisplayStr);
        ...
        {
                const std::vector<std::string> & launcherAvatarNames = ConfigClientGame::getLauncherAvatarNames ();

                if (!launcherAvatarNames.empty ())
                {
                        for (std::vector<std::string>::const_iterator launcherIt = launcherAvatarNames.begin (); launcherIt != launcherAvatarNames.end () && avatarNameToUse.empty (); ++launcherIt)
                        {
                                avatarNameToUse = *launcherIt;
                                const size_t splitpos = avatarNameToUse.find (" (");
                                if (splitpos != std::string::npos)
                                        avatarNameToUse = avatarNameToUse.substr (0, splitpos);

                                if (_stricmp (avatarNameToUse.c_str (), narrowName.c_str ()) != 0)
                                        avatarNameToUse.clear ();
                        }
                }

                if (avatarNameToUse.empty ())
                {
	...
	if (!m_autoConnected)
	{
		if (ConfigClientGame::getAutoConnectToGameServer ())
			return true;
                ...
                if (!ConfigClientGame::getLauncherAvatarNames ().empty ())
                        return true;
        }

...
{
		bool configFileExists(char const *filename)
		{
			DWORD const attributes = GetFileAttributesA(filename);
	...
	const char * configFileName = "client.cfg";
	{
		...
		if (strstr(programName, "testswgclient") != 0)
			configFileName = "test.cfg";
                ...
                char const * const envConfigFile = getenv("SWG_CONFIG_FILE");
                if (envConfigFile && envConfigFile[0] != '\0')
                        configFileName = envConfigFile;
                else if (configFileExists("Swg+ai.cfg"))
                        configFileName = "Swg+ai.cfg";
                else if (configFileExists("Swg+Setup.cfg"))
                        configFileName = "Swg+Setup.cfg";
                else if (configFileExists("Swg+Setup_d.cfg"))
                        configFileName = "Swg+Setup_d.cfg";
	}
	...
	else
	{
		...
		SetupClientGraphics::setupDefaultGameData (setupGraphicsData);
			...
			CuiManager::setImplementationTestFunction     (SwgCuiManager::test);
                    ...
                    // Bug reporting is not initialized for the headless client build,
                    // which keeps the dependency surface minimal for this lightweight target.

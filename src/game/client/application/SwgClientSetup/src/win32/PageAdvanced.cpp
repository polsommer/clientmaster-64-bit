...
void PageAdvanced::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_ALLOWMISSINGTOC, m_allowMissingTOC);
	...
	DDX_Text(pDX, IDC_CHECK_ALLOWMISSINGTOC, m_lblAllowMissingTOC);
...
BEGIN_MESSAGE_MAP(PageAdvanced, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_CHECK_ALLOWMISSINGTOC, OnCheckAllowmissingtoc)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void PageAdvanced::initializeDialog() 
{
	...
	VERIFY(m_lblAllowMissingTOC.LoadString(IDS_ADVANCED_ALLOW_MISSING_TOC));
	...
	m_allowMissingTOC = Options::getAllowMissingTOC ();
...
void PageAdvanced::applyOptions ()
{
	...
	Options::setAllowMissingTOC (m_allowMissingTOC == TRUE);
}
...
void PageAdvanced::OnCheckAllowmissingtoc() 
{
	applyOptions ();
}

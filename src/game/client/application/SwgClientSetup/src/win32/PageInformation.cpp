...
VERIFY(m_lblNumProcessors.LoadString(IDS_INFO_NUM_PROCESSORS));
	...
	VERIFY(Unknown.LoadString(IDS_UNKNOWN));
        ...
        m_physicalMemorySize.Format (_T("%I64u %s"), (ClientMachine::getPhysicalMemorySize()/1024+1), GB);
	...
	m_cpuIdentifier = ClientMachine::getCpuIdentifier ();
        ...
        if (ClientMachine::isWindows11OrGreater())
                m_os += _T(" (Windows 11+ detected)");
        else if (ClientMachine::isWindows10OrGreater())
                m_os += _T(" (Windows 10+ detected)");
	...
	m_videoDriverVersion = ClientMachine::getDeviceDriverVersionText ();
        ...
        m_videoMemorySize.Format (_T("%I64u %s"), static_cast<unsigned __int64>(ClientMachine::getVideoMemorySize ()/1024+1), GB);
	...
	if (ClientMachine::getPixelShaderMajorVersion () != 0)
        ...
        m_directXVersion.Format(_T("%s (DX12 %s; HDR %s; displays: %d)"),
                ClientMachine::getDirectXVersion(),
                ClientMachine::supportsDirectX12() ? _T("detected") : _T("not detected"),
                ClientMachine::hasHdrCapableDisplay() ? _T("available") : _T("off"),
                ClientMachine::getDxgiOutputCount());
